...
const MessagesView: React.FC<MessagesViewProps> = ({ userParticipations, allMeetings, onSelectChat }) => {
          ...
          >
            ...
            <div className="flex-1 flex flex-col gap-1 min-w-0">
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-bold text-teal-500 uppercase">{chat.category}</span>
                <span className="text-[10px] text-slate-300 font-medium">활동 중</span>
              </div>
              <h3 className="font-bold text-slate-800 truncate">{chat.title}</h3>
              <p className="text-[12px] text-slate-400 truncate">멤버들과 인사를 나눠보세요!</p>
            </div>
